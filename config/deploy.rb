...
namespace :puma do
  ...
  task :start, :roles => lambda { fetch(:puma_role) }, :on_no_matching_servers => :continue do
    ...
    run "cd #{current_path} && #{fetch(:puma_cmd)} -t 4:4 -q -e #{puma_env} -b 'unix://#{shared_path}/sockets/unicorn.sock' -S #{fetch(:puma_state)} --control 'unix://#{shared_path}/sockets/pumactl.sock' >> #{shared_path}/log/puma-#{puma_env}.log 2>&1 &", :pty => false
  end
